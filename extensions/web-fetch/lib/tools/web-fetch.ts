/**
 * Standalone web fetch tool.
 *
 * Fetches a URL and returns one of:
 * - raw HTML
 * - extracted plain text (via html-to-text)
 * - extracted "markdown" (via @mozilla/readability, then textified)
 */
...
export type WebFetchFormat = 'html' | 'text' | 'markdown'
...
Note: This tool does NOT render JavaScript.`,
    ...
    renderResult(
      ...
      const footerText =
        ...
        (footerParts.length
          ? theme.fg('muted', ` ${footerParts.join(' ')}`)
          : '')
